...
class PackageShoppingCartDA {

    public function getOnePackageByCustID($customerID, $conn) {
        $shoppingCart = new ShoppingCart();

        $stmt = $conn->prepare("SELECT `Customer_ID`, `Product_ID` FROM `package_shopping_cart` WHERE `Customer_ID`=?");
    ...
    public function insert($shoppingCart, $conn) {
        ...
        $stmt = $conn->prepare("INSERT INTO `package_shopping_cart`(`Customer_ID`, `Product_ID`) VALUES (?,?)");
    ...
    public function delete($customerID, $conn) {
        $stmt = $conn->prepare("DELETE FROM `package_shopping_cart` WHERE `Customer_ID`=?");
        $stmt->bind_param("s", $customerID);
        $stmt->execute();
        $stmt->close();
        unset($stmt);
